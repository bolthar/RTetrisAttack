...
#    Surface.blit(@tree, 0, 0, 0, 0, @screen, 0, 0)
  ...
  def render_falling(block, x, y, ticks)
    Surface.blit(get_tokens(y)[block.type],0,0,0,0 ,@screen,@start_x + (x*16), @end_y - (y*48) - (ticks * 3) + (block.state.counter * 24))
  end
  ...
  def render_effects(effects, x, y)
    ...
    effects.each do |effect|
      ...
      Surface.blit(surface, 0, 0, 0, 0, @screen, @start_x + (x*48), @end_y - (y*48) - 30 - offset + (6*(24 - (Math.sqrt(effect.counter)))))
      offset += 60
    end
  end
